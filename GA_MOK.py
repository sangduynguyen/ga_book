...
try:
 ...
 if (alpha > 0) & (alpha <1):
   ...
   alpha = 1e-1
   print('0<alpha<1, alpha=',alpha)
except:
 print("0<alpha<1",alpha)
...
def onclick(event):
    ...
    coord.append((event.ydata,event.xdata, z, alpha))
    x1 = event.xdata
    y1 = event.ydata
    #z = 1.61  
...
# amplitude a, position b, and width parameter c.
def Am(alpha):
    # return 2.6-1.9203*alpha+0.324*alpha**3.338
    return 2.6 - 1.9203 * alpha + 0.324 * alpha ** 3.338

def rm(alpha):
    return (1.0 - alpha) * (1.0 + 0.2922 * alpha - 0.2783 * alpha ** 2)
    # return (Am(alpha)+alpha)/(Am(alpha)-alpha)
...
def fx(x):
    ...
    b0 = 1.681534
    b1 = 3.330657
    return 1.0 - (a0 + a1 * x + x ** 2) / (b0 + b1 * x + x ** 2) + 4e-5

# defines GOK a typical gaussian function, of independent variable x,
# amplitude a, position b, width parameter c, and kinetic order bv
def E1m(x, b, c):
    return abs(np.exp(c / kbz / b - c / kbz / x))

def E2m(x, b, c, alpha):
    return abs(np.exp(
        rm(alpha) * c / kbz / b * ((x / b * np.exp(c / kbz / b - c / kbz / x) * fx(c / kbz / x)) - fx(c / kbz / b))))

def gaussian(x, a, b, c, alpha):
    # return (4*a*rm(alpha)**2*np.exp(c/kbz/b-c/kbz/x)*np.exp(rm(alpha)*c/kbz/b*(x/b*np.exp(c/kbz/b-c/kbz/x)*fx(c/kbz/x)-fx(c/kbz/b))))/(((1+rm(alpha))*(np.exp(rm(alpha)*c/kbz/b*(x/b*np.exp(c/kbz/b-c/kbz/x)*fx(c/kbz/x)-fx(c/kbz/b))))-(1-rm(alpha)))**2)
    return 4 * a * rm(alpha) ** 2 * E1m(x, b, c) * E2m(x, b, c, alpha) / (((1 + rm(alpha)) * E2m(x, b, c, alpha)) - (1 - rm(alpha))) ** 2
...
#defines ffMIX=s, remove x=T, a=Im, only b=Tm, c=E

def ffMIX(x,a,b,c,alpha):
    #return (hr*c)/(kbz*b**2)*np.exp(c/kbz/b)*rm(alpha)*alpha/(1-alpha)
    return abs((hr*c)/(kbz*b**2)*np.exp(c/kbz/b)*rm(alpha))

# defines half1GOK=s, remove x=T, a=Im, only b=Tm, c=E
def half1MIX(x,a,b,c,alpha):
    return abs((alpha/(1-alpha))*(kbz*b**2)*(1-alpha)*(1+N*alpha/n/(1-alpha))*np.log(2-alpha)/(hr*c*alpha*rm(alpha)))

# defines half2GOK=s, remove x=T, a=Im, only b=Tm, c=E
def half2MIX(x,a,b,c,alpha):
    return abs((alpha/(1-alpha))*(kbz*b**2)*(1-alpha)*(1+N*alpha/n/(1-alpha))*np.log(1-alpha+alpha**2+(1-alpha)*np.sqrt(1+alpha**2))/(hr*c*alpha*rm(alpha)))
...
def x_function(x,a,b,c,alpha):
    #a,b,c,alpha=10968.19, 473, 1.1, 0.001
    kbz = 8.617385e-5
    #return a*np.exp(1.0+c/kbz/x*((x-b)/b)-((x/b)**2)*np.exp(c/kbz/x*((x-b)/b))*(1.0-2.0*kbz*x/c)-2.0*kbz*b/c)
    ...
    return abs(4 * a * rm(alpha) ** 2 * E1m(x, b, c) * E2m(x, b, c, alpha) / (((1 + rm(alpha)) * E2m(x, b, c, alpha)) - (1 - rm(alpha))) ** 2)
...
def roulette(values, fitness):
	...
	return i	
# Func GA
def geneticTL(x,a,b,c,alpha):
    # Genetic Algorithm Code to find the Maximum of F(X)
    
    #x_function(x,a,b,c,alpha)
    #Range of Values
    #x_max = 32000
    x_max = 600
    #x_min = 0
    ...
    for i in range(pop_size):
    	...
    	f_tmp = x_function(x_tmp,a,b,c,alpha)
    ...
    for i in range(number_of_generations):
    	...
    	#Creating Generation 2
    		...
    		new_f_AB = x_function(new_AB,a,b,c,alpha)
    		gen_2_fvalues.append(new_f_AB)
    
    		new_BA = int(ind_BA,2)
    		gen_2_xvalues.append(new_BA)
    
    		new_f_BA = x_function(new_BA,a,b,c,alpha)
    	...
    	fitness = 0
    	for j in range(pop_size):
    		f_tmp = x_function(gen_1_xvalues[j],a,b,c,alpha)
    		fitness += f_tmp
...
def AE_gen(x,a,b,c,alpha):
    #x = data_set[:,0]
    y=gaussian(x,a,b,c,alpha)
    Tm = geneticTL(x,a,b,c,alpha)
    Im = geneticTLy(x,a,b,c,alpha)
